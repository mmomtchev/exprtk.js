...
#define ASYNCABLE_DEFINE(method)                                                                                                                               \
    Napi::Value method(const Napi::CallbackInfo &info) { return method##_do(info, false); }                                                                    \
    Napi::Value method##Async(const Napi::CallbackInfo &info) { return method##_do(info, true); }                                                              \
...
#define ASYNCABLE_INSTANCE_METHOD(klass, method, props)                                                                                                        \
    klass::InstanceMethod(#method, &klass::method, props), klass::InstanceMethod(#method "Async", &klass::method##Async, props)
...
template <class T> class ExprTkAsyncWorker : public Napi::AsyncWorker {
  public:
    ...
    explicit ExprTkAsyncWorker(Napi::Function &callback, const MainFunc &doit, const RValFunc &rval, const std::map<std::string, Napi::Object> &objects);
...
template <class T>
ExprTkAsyncWorker<T>::ExprTkAsyncWorker(Napi::Function &callback, const MainFunc &doit, const RValFunc &rval,
                                        const std::map<std::string, Napi::Object> &objects)
    : AsyncWorker(callback, "ExprTk.js:ExprTkAsyncWorker"), doit(doit), rval(rval) {

    for (auto i = objects.begin(); i != objects.end(); i++)
        persistent[i->first] = Napi::Persistent(i->second);
}
...
template <class T> void ExprTkAsyncWorker<T>::OnOK() { Callback().Call({Env().Null(), rval(raw)}); }
...
template <class T> class ExprTkJob {
  public:
    ...
    ExprTkJob() : main(), rval(), persistent(), autoIndex(0){};

    inline void persist(const std::string &key, const Napi::Object &obj) { persistent[key] = obj; }

    inline void persist(const Napi::Object &obj) { persistent[std::to_string(autoIndex++)] = obj; }
    ...
    Napi::Value run(const Napi::CallbackInfo &info, bool async, int cb_arg) {
        if (!info.This().IsEmpty() && info.This().IsObject())
            persist("this", info.This().As<Napi::Object>());
        if (async) {
            ...
            auto worker = new ExprTkAsyncWorker<T>(callback, main, rval, persistent);
            worker->Queue();
            return info.Env().Undefined();
        }
        try {
